    ...
    for(int run=0; run<kRunNum; run++){
        ...
        for(int type=0; type<kTypeNum; type++){
            for(int dle=0; dle<kDLENum; dle++){
                ...
                latex -> DrawLatexNDC(0.6, 0.65, Form("<M> = %.2f", mFitting->GetMassMean(run, type, dle)));
                latex -> DrawLatexNDC(0.6, 0.59, Form("#sigma = %.2f", mFitting->GetMassSigma(run, type, dle)));
                latex -> DrawLatexNDC(0.6, 0.53, Form("All N = %i", int(mFitting->GetMassAllCounts(run, type, dle))));
                latex -> DrawLatexNDC(0.6, 0.47, Form("Sig. N = %i", int(mFitting->GetMassSignalCounts(run, type, dle))));
                latex -> DrawLatexNDC(0.6, 0.42, Form("Bkg. N = %i", int(mFitting->GetMassBkgCounts(run, type, dle))));
            ...
            for(int dle=0; dle<kDLENum; dle++){
                ...
                for(int pt=0; pt<ptNum; pt++){
                    for(int xf=0; xf<xfNum; xf++){
                        ...
                        latex -> DrawLatexNDC(0.6, 0.65, Form("<M> = %.2f", mFitting->GetMassMean(run, type, dle, pt, xf)));
                        latex -> DrawLatexNDC(0.6, 0.59, Form("#sigma = %.2f", mFitting->GetMassSigma(run, type, dle, pt, xf)));
                        latex -> DrawLatexNDC(0.6, 0.53, Form("All N = %i", int(mFitting->GetMassAllCounts(run, type, dle, pt, xf))));
                        latex -> DrawLatexNDC(0.6, 0.47, Form("Sig. N = %i", int(mFitting->GetMassSignalCounts(run, type, dle, pt, xf))));
                        latex -> DrawLatexNDC(0.6, 0.42, Form("Bkg. N = %i", int(mFitting->GetMassBkgCounts(run, type, dle, pt, xf))));
